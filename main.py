import sys
import arcade
from views.game_view import SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE
...
def run_game_over(result=True, score="0", timer="00:00", statistic="Игра завершена"):
    window = arcade.Window(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE)
    game_over_view = Game_Over_View(result, score, timer, statistic)
    window.show_view(game_over_view)
    arcade.run()


if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "game_over":
